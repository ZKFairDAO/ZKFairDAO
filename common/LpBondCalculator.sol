...
contract LpBondCalculator is IBondingCalculator {
  ...
  function getKValue(address _pair) public view returns (uint256 k_) {
    ...
    (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(_pair)
      .getReserves();
    uint256 decimals = 0;
    if (token0.add(token1) > IERC20(_pair).decimals()) {
      decimals = token0.add(token1).sub(IERC20(_pair).decimals());
  ...
  function markdown(address _pair) external view returns (uint256) {
    ...
    uint256 reserve;
    if (IUniswapV2Pair(_pair).token0() == ZKFD) {
      reserve = reserve1;
    } else {
      reserve = reserve0;
    }
    return
      reserve.mul(2 * (10**IERC20(ZKFD).decimals())).div(getTotalValue(_pair));
  }
}
