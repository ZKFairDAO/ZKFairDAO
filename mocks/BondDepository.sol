...
contract BondDepository is Ownable {
    ...
    address public immutable ZKFD; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints ZKFD when receives principle
    ...
    struct Bond {
        uint256 payout; // ZKFD remaining to be paid
    ...
    function setAdjustment(
        ...
        require(
            _increment <= terms.controlVariable.mul(50).div(100),
            "Increment too large"
        );
    ...
    function deposit(
        ...
        // must be > 0.0001 ZKFD ( underflow protection )
        ...
        /**
                ...
                approved and
                deposited into the treasury, returning (_amount - profit) ZKFD
             */
        ...
        if (fee != 0) {
            ...
            IERC20(ZKFD).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(ZKFD).transfer(_recipient, _amount);
            // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(ZKFD).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(ZKFD).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(ZKFD).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to ZKFD supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(ZKFD).totalSupply();
    ...
    /**
     *  @notice debt ratio in terms for reserve or liquidity bonds
     *  @return uint
     */
    ...
    /**
     *  @notice calculate amount of ZKFD available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or ZKFD) to the DAO
     *  @return bool
     */
