...
contract DepositHelper is Ownable {
  ...
  function depositHelper(
    ...
    if (_tokenAddress == principle) {
      ...
      require(
        _tokenAddress == _tokenA || _tokenAddress == _tokenB,
        "_tokenAddress err"
      );
      ...
      ZKFD.safeApprove(address(router), uint256(-1));
      ...
      if (ZKFD.myBalance() > 0) {
        ZKFD.safeTransfer(msg.sender, ZKFD.myBalance());
      }
  ...
  function depositValue(uint256 _amount) public view returns (uint256 value_) {
    ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(USDC, ZKFD));
    (uint256 token0Reserve, uint256 token1Reserve, ) = lpToken.getReserves();
    (uint256 debtReserve, uint256 relativeReserve) = USDC == lpToken.token0()
    ...
    if (swapAmt > 0) {
      ...
      (path[0], path[1]) = isReversed ? (ZKFD, USDC) : (USDC, ZKFD);
      uint256[] memory amounts = router.getAmountsOut(swapAmt, path);
      (uint256 amount0, uint256 amount1) = USDC == lpToken.token0()
        ? (_amount.sub(swapAmt), amounts[1])
        : (amounts[1], _amount.sub(swapAmt));
      uint256 _totalSupply = lpToken.totalSupply();
      uint256 lpAmount = Math.min(
        amount0.mul(_totalSupply) / token0Reserve,
        amount1.mul(_totalSupply) / token1Reserve
      );
  ...
  function calAndSwap(
    ...
    (uint256 token0Reserve, uint256 token1Reserve, ) = lpToken.getReserves();
    (uint256 debtReserve, uint256 relativeReserve) = _tokenA == lpToken.token0()
      ? (token0Reserve, token1Reserve)
      : (token1Reserve, token0Reserve);
    (uint256 swapAmt, bool isReversed) = optimalDeposit(
      _tokenA.myBalance(),
